<?php 
	...
	$m .= '</select>';
?>
<div style="width:500px; height:150px; overflow:auto; color:#000000; margin-bottom:20px;" align="center">
	...
	$('.save').click(function(){
		if($('#event').val().length > 0){
			$.ajax({
				type: 'POST',
				dataType: 'json',
				url: "<?php echo site_url("/evencal/evencal/do_add");?>",
				data:{<?php echo "year:$year,mon:$mon,day:$day";?>, hour:$('#hour').val(), minute: $('#minute').val(), event:$('#event').val()},
				success: function(data) {
					if(data.status){
						//$.fn.colorbox.close(data);
						window.location = '<?php echo site_url("/evencal/evencal/detail/$year/$mon/$day")?>';
					}else{
